<template>
    <div>
        ...
        <div v-if="cartItem.supGoodsList.length>0 && cartItem.activityPower.length>0" class="header">
            <div style="flex-direction: row;justify-content: space-between;align-items: center;border-top-color:#f0f0f0;border-top-width:20px;border-bottom-color:#f0f0f0;border-bottom-width:2px;">
                <div style="flex-direction: row;align-items: center;flex:1;">
                    <div class="select-mark" @click="selectHeaderClick()">
                        <image v-if="cartItem.selected == undefined || cartItem.selected" class="select-img" resize="contain" src="/web/assets/sup/sup_cart_check.png" @click="selectHeaderClick()" />
                        <image v-else class="select-img" resize="contain" src="/web/assets/sup/sup_cart_uncheck.png" @click="selectHeaderClick()" />
                    </div>
                    <text style="height:30px;color:#ff0033;font-size:18px;padding-left:5px;padding-right:5px;border-color:#ff0033;border-width:2px;border-radius:5px;line-height: 26px;">满减</text>
                    <text style="font-size:24px;padding:20px;flex:1;">{{ headerShowInfo() }}</text>
                </div>
                <div v-if="showCouDan" style="flex-direction: row;align-items: center;padding-right:16px;">
                    <text style="color:#ff0033;font-size:20px;" @click="addOnItemAction(cartItem.subjectId)">去凑单</text>
                    <wxc-icon name="more" :icon-style="{color:'#ff0033',fontWeight: 'bolder',fontSize:'26px'}" @wxcIconClicked="addOnItemAction(cartItem.subjectId)"></wxc-icon>
                </div>
            </div>
        </div>
        <div v-else class="grap"></div>
        <!-- cell -->
                ...
                <div class="cell-content">
                    ...
                    <div style="margin-left:20px ;margin-right:20px ;flex:1 ;">
                        ...
                        <div style="flex-direction: row;justify-content: space-between;align-items: center;">
                            ...
                            <text>x{{ goodsItem.count }}</text>
                        </div>
                    </div>
                    <div class="mask">
                        <div v-if="editEnable" class="select-mark" @click="selectCellClick(goodsItem)">
                            <image v-if="goodsItem.selected == undefined || goodsItem.selected" class="select-img" resize="contain" src="/web/assets/sup/sup_cart_check.png" @click="selectCellClick(goodsItem)" />
                            <image v-else class="select-img" resize="contain" src="/web/assets/sup/sup_cart_uncheck.png" @click="selectCellClick(goodsItem)" />
                        </div>
                        <div v-else class="select-mark"></div>
                        <text class="invalid-text"> 已失效 </text>
                    </div>
                </div>
            </div>
            <div v-else class="cell-content">
                <div class="select-mark" @click="selectCellClick(goodsItem)">
                    <image v-if="goodsItem.selected == undefined || goodsItem.selected" class="select-img" resize="contain" src="/web/assets/sup/sup_cart_check.png" @click="selectCellClick(goodsItem)" />
                    <image v-else class="select-img" resize="contain" src="/web/assets/sup/sup_cart_uncheck.png" @click="selectCellClick(goodsItem)" />
                </div>
                <image style="width:206px;height:156px;background-color:#f0f0f0;" :src="goodsItem.mainImgURL" />
                <div style="margin-left:20px;margin-right:20px; flex:1 ;">
                    ...
                    <div style="flex-direction: row;justify-content: space-between;align-items: center;">
                        ...
                        <div style="flex-direction: row;align-items: center;">
                            <div style="width:72px;height:42px;align-items: center;" @click="reduceGoodsCountClick(goodsItem)">
                                <image style="width:42px;height:42px;" src="/web/assets/sup/sup_cart_reduce.png" @click="reduceGoodsCountClick(goodsItem)" />
                            </div>
                            <text>{{ goodsItem.count }}</text>
                            <div style="width:72px;height:42px;align-items: center;" @click="addGoodsCountClick(goodsItem)" >
                                <image style="width:42px;height:42px;" src="/web/assets/sup/sup_cart_add.png" @click="addGoodsCountClick(goodsItem)" />
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div v-if="cartItem.invalidGoodsList" class="invalid-cell" v-for="(invalidGoods,y) in cartItem.invalidGoodsList" :key="y">
            <div class="cell-content">
                ...
                <div class="mask">
                    <div class="select-mark">
                        <image class="select-img" resize="contain" src="/web/assets/sup/sup_cart_check.png" />
                    </div>
    ...
    methods: {
        ...
        reduceGoodsCountClick:function(goodsModel){
            if(goodsModel.count>1){
                ...
                // 写在下面是有原因的(因为2->1时，有删除操作，放在前面会提前进行删除操作)
        ...
        addGoodsCountClick:function(goodsModel){
            ...
            if(Number(goodsModel.count) < Number(goodsModel.supGoodsInventory)){
                goodsModel.count = parseInt(goodsModel.count) + 1;
        ...
        changeGoodsCountEvent:function(objc,isAdd){
            const self = this;
            if (self.jessionId == 'undefined' || self.jessionId.length == 0){
