<template>
    <wxc-mask :height="maskHeight" width="680" border-radius="10" duration="200" 
        ...
        <div>
            ...
            <div class="bottom-btns" style="flex-direction: row;height:100px;border-top-color:#f0f0f0;border-top-width:2px;margin-top:20px;">
    ...
    computed: {
        ...
        maskHeight:function(){
            ...
            if(WXEnvironment.platform === 'android'){
                maskHeight = self.warn?'370':'560'
            }else{
                maskHeight = self.warn?'410':'620' 
            }
            return maskHeight;
        },
    },
    methods: {
        cancelBtnClicked: function() {
            ...
            if(self.warn){
                ...
                // self.exit(1);
                if (WXEnvironment.platform === 'android') {
                    weex.requireModule('myGlobalEvent').postNotificationName('exit',{agree:'false'})
                } else {
                    globalEvent.postNotificationName('exit',{agree:'false'})
                }
            }else{
        ...
        confirmBtnClicked: function() {
            ...
            storage.setItem("Agree_Privacy","Agree_Privacy",event=>{
                if (WXEnvironment.platform === 'android') {
                    weex.requireModule('myGlobalEvent').postNotificationName('exit',{agree:'true'})
                } else {
                    globalEvent.postNotificationName('exit',{agree:'true'})
                }
            })
        },
        onclick:function(){
