        ...
        <scroller class="wrapper">  
            <div class="content">
                ...
                <div class="redeemCode-div">
                    <input ref="code_input" type="text" class="input-number" placeholder="输入兑换码" @change="codechange" @input="codeinput" />
                </div>
...
export default {
    ...
    data() {
        return {
            loading: false,
            topImg: "/web/assets/sup/sup_accountBalance_exchange_element.png",
            redeemMoney: '666',
            codeText: "",
            ruleBgImg: '',
            ruleString: '兑换规则\n1、请输入邀请码或兑换码进行兑换；\n2、同一个码同一个用户不可重复兑换；',
    ...
    methods: {
        ...
        codechange: function(event) {
            ...
            console.log("codeNumber:codechange", event.value);
        },
        codeinput: function(event) {
            this.codeText = event.value;
            console.log("codeNumber:codeinput", event.value);
        },
        exchange: function () {
            console.log("立即兑换")
            ...
            if (enable) {
                ...
                return;
            } else if (check.isEmojiCharacter(this.codeText)) {
                modal.toast({
                    message:"兑换码不能包含特殊字符\n请输入正确的兑换码",
                    duration: 1.0
                })
